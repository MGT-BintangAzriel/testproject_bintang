...
/**
 * 書籍申請のヘッダー情報をDBに登録・取得・更新するリポジトリクラス。
 *
 * @author MGT-BintangAzriel
 * @version 1.0.0
 */
public class BookHeaderRepository {

	// 処理対象テーブル名
	private String tableName = "wf_book_header";
	...
	/**
	 * 書籍申請のヘッダー情報をDBに登録するメソッド。
	 *
	 * @param varDataHeaderData 登録対象のヘッダー情報モデル
	 * @throws Exception 登録処理中に例外が発生した場合
	 */
	public void insertDataHeader(BookHeaderModel varDataHeaderData) throws Exception {
		try {
	 ...
	 */
	private ColumnValues setDataHeaderValue(BookHeaderModel varDataHeaderData, String Condition) {
	 ...
	 */
	public Collection<BookHeaderModel> selectDataHeader(String select_value, String select_where) throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
			...
			// DBから書籍申請のヘッダー情報を取得する
			Collection<BookHeaderModel> result = sqlManager.select(BookHeaderModel.class, select_query, parameters);
			return result;
	...
	/**
	 * DBの書籍申請のヘッダー情報を更新するメソッド。
	 *
	 * @param varDataHeaderData 更新対象のヘッダー情報モデル
	 * @throws Exception 更新処理中に例外が発生した場合
	 */
	public void updateDataHeader(BookHeaderModel varDataHeaderData) throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
