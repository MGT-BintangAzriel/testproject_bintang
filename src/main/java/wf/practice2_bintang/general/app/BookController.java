...
import wf.practice2_bintang.general.domain.service.BookWorkflowService;

/**
 * 書籍申請の画面遷移を制御するコントローラクラス。
 *
 * @author MGT-BintangAzriel
 * @version 1.0.0
 */
@Controller
@RequestMapping("practice2_bintang/")
public class BookController {
    ...
    // JSPの共通パス
    private static final String BASE_VIEW_PATH = "wf/practice2_bintang/general/";
    // 申請画面のJSPパス
    ...
    public final String apply(final Model model, final BookForm ApplyForm) throws Exception {
        ...
        if (PageType.pageTyp_App.toString().equals(ApplyForm.getImwPageType())) {
            ...
            BookForm FormClassRows = new BookForm();
    ...
    public final String approve(final Model model, final BookForm ApplyForm)
        ...
        try {
            // 過去のデータをシステム案件IDで取得
            BookWorkflowService service = new BookWorkflowService();
            BookForm formRows = new BookForm();
    ...
    public final String detail(final Model model, final BookForm ApplyForm)
            throws AccessSecurityException, IOException {
