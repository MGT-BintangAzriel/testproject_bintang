...
/**
 * 備品申請のヘッダー情報をDBに登録・取得・更新するリポジトリクラス。
 *
 * @author MGT-BintangAzriel
 * @version 1.0.0
 */
public class EquipmentHeaderRepository {

	// 処理対象テーブル名を定数化する
	private String tableName = "wf_equipment_header";
	...
	/**
	 * 備品申請のヘッダー情報をDBに登録するメソッド。
	 *
	 * @param varDataHeaderData 登録対象のヘッダー情報モデル
	 * @throws Exception 登録処理中に例外が発生した場合
	 */
	public void insertDataHeader(EquipmentHeaderModel varDataHeaderData) throws Exception {
		try {
	 ...
	 */
	private ColumnValues setDataHeaderValue(EquipmentHeaderModel varDataHeaderData, String Condition) {
	 ...
	 */
	public Collection<EquipmentHeaderModel> selectDataHeader(String select_value, String select_where)
			throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
			...
			// DBから備品申請のヘッダー情報を取得する
			Collection<EquipmentHeaderModel> result = sqlManager.select(EquipmentHeaderModel.class, select_query,
					parameters);
			return result;
	...
	/**
	 * DBの備品申請のヘッダー情報を更新するメソッド。
	 *
	 * @param varDataHeaderData 更新対象のヘッダー情報モデル
	 * @throws Exception 更新処理中に例外が発生した場合
	 */
	public void updateDataHeader(EquipmentHeaderModel varDataHeaderData) throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
