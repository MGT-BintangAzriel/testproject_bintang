...
/**
 * 経費申請のヘッダー情報をDBに登録・取得・更新するリポジトリクラス。
 *
 * @author MGT-BintangAzriel
 * @version 1.0.0
 */
public class ExpenseHeaderRepository {

	// 処理対象テーブル名
	private String tableName = "wf_expense_header";
	...
	private String selectDataMail = "SELECT * FROM wf_expense_header WHERE status = '2' and mail_status = '1'";
	 ...
	 */
	public void insertDataHeader(ExpenseHeaderModel varDataHeaderData) throws Exception {
		try {
	 ...
	 */
	private ColumnValues setDataHeaderValue(ExpenseHeaderModel varDataHeaderData, String Condition) {
	 ...
	 */
	public Collection<ExpenseHeaderModel> selectDataHeader(String select_value, String select_where) throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
			...
			Collection<ExpenseHeaderModel> result = sqlManager.select(ExpenseHeaderModel.class, select_query,
					parameters);
			return result;
	 ...
	 */
	public void updateDataHeader(ExpenseHeaderModel varDataHeaderData) throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
