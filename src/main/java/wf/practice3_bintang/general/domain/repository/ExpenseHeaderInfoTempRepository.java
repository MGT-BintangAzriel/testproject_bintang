...
/**
 * 経費申請一時保存情報をDBに登録・取得・更新するリポジトリクラス。
 *
 * @author MGT-BintangAzriel
 * @version 1.0.0
 */
public class ExpenseHeaderInfoTempRepository {

	// 処理対象テーブル名
	private String tableName = "wf_expense_header_info_temp";
	...
	/**
	 * 経費申請一時保存情報をDBに登録するメソッド。
	 *
	 * @param varDataHeaderData 登録対象の一時保存情報モデル
	 * @throws Exception 登録処理中に例外が発生した場合
	 */
	public void insertDataHeader(ExpenseHeaderInfoTempModel varDataHeaderData) throws Exception {
		try {
	 ...
	 */
	private ColumnValues setDataValue(ExpenseHeaderInfoTempModel varDataHeaderData, String Condition) {
		...
		if (Condition.equals(WorkflowCommonConstants.CONDITION_CREATE)) {
			...
			result.add(ExpenseDbConstants.COLUMN_EXPENSE_CONTENT, varDataHeaderData.getExpense_content());
			result.add(ExpenseDbConstants.COLUMN_PRICE, varDataHeaderData.getPrice());
			result.add(ExpenseDbConstants.COLUMN_REASON, varDataHeaderData.getReason());
	 ...
	 */
	public Collection<ExpenseHeaderInfoTempModel> selectDataInfoTempHeader(String select_value, String select_where)
			throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
			...
			// DBから経費申請一時保存情報を取得する
			Collection<ExpenseHeaderInfoTempModel> result = sqlManager.select(ExpenseHeaderInfoTempModel.class,
					select_query,
					parameters);
			return result;
	...
	/**
	 * DBの経費申請一時保存情報を更新するメソッド。
	 *
	 * @param varDataHeaderData 更新対象の一時保存情報モデル
	 * @throws Exception 更新処理中に例外が発生した場合
	 */
	public void updateDataHeaderInfoTemp(ExpenseHeaderInfoTempModel varDataHeaderData) throws Exception {
		try {
			SQLManager sqlManager = new SQLManager();
			...
			searchCondition.addCondition(WorkflowCommonConstants.COLUMN_SYSTEM_MATTER_ID, varDataHeaderData.getSystem_matter_id());
