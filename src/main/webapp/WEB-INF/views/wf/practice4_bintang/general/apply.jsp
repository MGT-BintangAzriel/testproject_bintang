...
<imui:head>
	...
	<script type="text/javascript">
		$(function() {
			...
			// イベントハンドラの設定
			$('#f_quantity, #f_unit_price').on('input change', calculateTotalPrice);
			$('#f_pc_use_type').on('change', togglePcUseOther);
			...
			// 入力チェックルール定義
			var rules = {
				f_apply_date: { required: true },
				f_applicant: { required: true },
				f_department: { required: true },
				f_pc_use_type: { required: true },
				f_pc_use_other: {
					required: function() {
						return $('#f_pc_use_type').val() === 'その他';
					}
				},
				f_manufacturer: { required: true },
				f_model_number: { required: true },
				f_quantity: { required: true, min: 1 },
				f_unit_price: { required: true, min: 0 }
			};

			var messages = {
				f_apply_date: { required: "申請日を入力してください。" },
				f_applicant: { required: "申請者を入力してください。" },
				f_department: { required: "部署を入力してください。" },
				f_pc_use_type: { required: "PC用途を選択してください。" },
				f_pc_use_other: { required: "「その他」選択時はその他用途の入力が必須です。" },
				f_manufacturer: { required: "メーカーを入力してください。" },
				f_model_number: { required: "型番を入力してください。" },
				f_quantity: { required: "数量を入力してください。", min: "数量は1以上を入力してください。" },
				f_unit_price: { required: "単価を入力してください。", min: "単価は0以上を入力してください。" }
			};
			...
			$('#openPage').click(function() {
				...
				if (valid) {
					...
					imuiShowErrorMessage('入力内容にエラーがあります。確認してください。', [], true, 2500, false);
				}
			});
		});
	</script>
				...
				<table class="imui-form tab_header">
					<tbody>
						<tr>
							...
							<td>
								<input type="text" id="f_apply_date" name="f_apply_date" class="imuiCalendar"
									value="${FormClassRows.getF_apply_date()}" style="height: 20px;" placeholder="yyyy/MM/dd">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" id="f_applicant" name="f_applicant"
									value="${FormClassRows.getF_applicant()}" size="40" style="height: 20px;">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" id="f_department" name="f_department"
									value="${FormClassRows.getF_department()}" size="40" style="height: 20px;">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<select id="f_pc_use_type" name="f_pc_use_type" style="height: 25px;">
						...
						<tr>
							...
							<td>
								<input type="text" id="f_pc_use_other" name="f_pc_use_other"
									value="${FormClassRows.getF_pc_use_other()}" size="50" style="height: 20px;">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" id="f_manufacturer" name="f_manufacturer"
									value="${FormClassRows.getF_manufacturer()}" size="40" style="height: 20px;">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" id="f_model_number" name="f_model_number"
									value="${FormClassRows.getF_model_number()}" size="40" style="height: 20px;">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="number" id="f_quantity" name="f_quantity"
									value="${FormClassRows.getF_quantity()}" min="1" style="height: 20px;">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="number" id="f_unit_price" name="f_unit_price"
									value="${FormClassRows.getF_unit_price()}" min="0" style="height: 20px;"> 円
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<textarea id="f_remarks" name="f_remarks" rows="3" cols="50">${FormClassRows.getF_remarks()}</textarea>
							</td>
						</tr>
					</tbody>
				</table>
