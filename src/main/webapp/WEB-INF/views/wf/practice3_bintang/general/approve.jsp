...
<imui:head>
	...
	<script type="text/javascript">
		$(function() {
			var rules = {
				f_expense_content: {required: true},
				f_price : {required: true}, 
				f_reason : {required: true},
			};
			
			var messages = {
				f_expense_content: {required: "申請内容を入力してください。" },
					...
					<tbody>
						<tr>
							<th width="250"><label class="imui-required"
								style="margin-left: 20px">申請内容</label></th>
							<td><input type="text" readonly value="${FormClassRows.getF_expense_content()}"  id="f_expense_content"
								name="f_expense_content"
					...
					<table class="imui-form">
						<tbody>
							<c:forEach items="${FormClassRows.d_list_attachment}" var="file">
								<tr>
									<td>
										<a href="practice3_bintang/download/${file.id}">${file.file_name}</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
